...
<form action='/admin/facilities/{{ $item->id }}' method='POST' class='bg-slate-900 border border-slate-700 bg-slate-950 text-slate-100 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 border-slate-800 shadow-xl p-6 rounded shadow max-w-2xl'>
    @csrf @method('PUT')
...
<div class='mb-4'><label class='block mb-1'>Icon name</label><input type='text' name='icon_name' value='{{ $item->icon_name }}' class='w-full border border-slate-700 bg-slate-950 text-slate-100 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 p-2 rounded'></div>
