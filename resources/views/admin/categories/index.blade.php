...
<div class='flex justify-between items-center mb-6'>
    <h2 class='text-2xl font-bold'>Manage Categorys</h2>
    <a href='/admin/categories/create' class='bg-indigo-600 text-white font-medium shadow-lg shadow-indigo-500/20 px-4 py-2 rounded shadow hover:bg-indigo-500 transition-colors duration-200'>Add New</a>
</div>
    ...
    <table class='w-full text-sm'>
        <thead class='bg-slate-800 border-b border-slate-800 border-slate-700'>
            <tr>
                ...
                <th class='p-3 text-left font-medium text-slate-400 uppercase tracking-wider text-xs'>Description</th>
        ...
        <tbody>
            ...
            <tr class='border-b border-slate-800 hover:bg-slate-800/50'>
                ...
                <td class='p-3 flex gap-2'>
                    <a href='/admin/categories/{{ $item->id }}/edit' class='text-indigo-400 transition-colors duration-200 hover:underline'>Edit</a>
                    <form action='/admin/categories/{{ $item->id }}' method='POST' onsubmit='return confirm("Delete this item?")'>
                        @csrf @method('DELETE')
