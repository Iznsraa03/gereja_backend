...
<form action='/admin/categories/{{ $item->id }}' method='POST' class='bg-white p-6 rounded shadow max-w-2xl'>
    @csrf @method('PUT')
...
<div class='mb-4'><label class='block mb-1'>Description</label><textarea name='description' class='w-full border p-2 rounded'>{{ $item->description }}</textarea></div>
<div class='mb-4'><label class='block mb-1'>Sort order</label><input type='number' name='sort_order' value='{{ $item->sort_order }}' class='w-full border p-2 rounded'></div>
