...
<form action='/admin/activities/{{ $item->id }}' method='POST' class='bg-white p-6 rounded shadow max-w-2xl'>
    @csrf @method('PUT')
...
<div class='mb-4'><label class='block mb-1'>Location name</label><input type='text' name='location_name' value='{{ $item->location_name }}' class='w-full border p-2 rounded'></div>
<div class='mb-4'><label class='block mb-1'>Start at</label><input type='datetime-local' name='start_at' value='{{ $item->start_at }}' class='w-full border p-2 rounded'></div>
<div class='mb-4'><label class='block mb-1'>End at</label><input type='datetime-local' name='end_at' value='{{ $item->end_at }}' class='w-full border p-2 rounded'></div>
